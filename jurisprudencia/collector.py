...
class TCMSPAdapter(JurisprudenciaAdapter):
    ...
    def _parse_records(raw: bytes, base_url: str, query: str) -> list[JurisprudenciaRecord]:
        ...
        for anchor in soup.find_all('a', href=True):
            ...
            match = re.search(r'/Documento/(TC)(\d{6})(\d{4})$', path, re.I)
            process = f'TC/{match.group(2)}/{match.group(3)}' if match else _extract_process(text, absolute)
            if not process:
    ...
    def _enrich(self, records: list[JurisprudenciaRecord], *, detail: bool, with_content: bool) -> list[JurisprudenciaRecord]:
        ...
        for record in records:
            try:
                ...
                print(f'aviso: detalhe TCM-SP indisponível para {record.numero_processo}: {type(exc).__name__}: {exc}')
        return records
