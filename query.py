...
def parse_filters(raw):
    return FILTER_RE.sub('', raw).strip(), dict(FILTER_RE.findall(raw))
...
def rerank(reranker, query, points):
    if not points:
        return []
    scores = list(reranker.rerank(query, [point.payload['text'] for point in points]))
    ...
    for point, _ in ranked:
        ...
        if len(output) >= config.FINAL_K:
            break
    return sorted(output, key=lambda point: (roles.get(point.payload.get('source_role'), 5), point.payload.get('authority_level') or 9))
...
def context(points):
    ...
    for index, point in enumerate(points, 1):
        ...
        part = (
            f"[F{index}] {payload['source']}, {page_label}{unit_ref} | "
...
def main():
    ...
    while True:
        ...
        try:
            ...
            answer = llm.generate(system_prompt=SYSTEM_PROMPT.format(context=context(points)), user_prompt=query)
        ...
        for index, point in enumerate(points, 1):
            print(f"[F{index}] {point.payload['source']} (p. {point.payload.get('page')})")
