...
HEADERS = {"User-Agent": "rag-licitacoes-source-sync/3.0 (+https://github.com/danihmorais/rag-licitacoes)", "Accept": "text/html,application/xhtml+xml,application/pdf;q=0.9,*/*;q=0.8"}
...
def discover_links(raw_html,base_url,source):
    soup=BeautifulSoup(raw_html,"html.parser"); patterns=[re.compile(normalized_pattern(p),re.I) for p in source.get("follow_patterns",())]; host=urlparse(base_url).netloc.lower(); out=[]; seen=set()
...
def cleanup_source_cache(source_id,keep_document_ids):
    ...
    for sidecar in CACHE.glob("*.json"):
        ...
        if meta.get("parent_source_id") != source_id or str(meta.get("document_id") or "") in keep: continue
        sidecar.unlink(missing_ok=True); sidecar.with_suffix(".txt").unlink(missing_ok=True); removed += 1
    return removed
