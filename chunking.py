...
def _default_tokenizer():
    try:
        ...
        print(f"Aviso: tokenizer do embedding indisponível no chunking; fallback por caracteres: {exc}")
        return None
...
def _excerpt_caput(caput, budget, tokenizer=None):
    ...
    left_budget = max(1, int(payload*0.30)); right_budget = max(1, payload-left_budget)
    left = _truncate_words_to_tokens(caput,left_budget,tokenizer)
    words = list(re.finditer(r"\S+", caput)); lo,hi,best = 0,len(words),""
    while lo <= hi:
        mid=(lo+hi)//2
...
def _jurisprudencia_units(text):
    ...
    for i,m in enumerate(matches):
        start=m.start(); end=matches[i+1].start() if i+1<len(matches) else len(text); value=text[start:end].strip()
        process=re.search(r"^PROCESSO:\s*(.+)$",value,re.I|re.M); sumula=re.search(r"^S[ÚU]MULA:\s*(.+)$",value,re.I|re.M)
...
def _build_ai_semantic_chunks(full_text,max_size,metadata,semantic_provider=None,tokenizer=None):
    ...
    try:
        ...
        from llm.semantic_chunker import SemanticChunkingError
